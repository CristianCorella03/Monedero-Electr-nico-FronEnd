<template>
    <div style="margin: 40px 50px;">
        <h1>Historial de Ventas | {{ establecimiento.nombre }} - {{ establecimiento.codigoEstablecimiento }}</h1>
        <hr>
        <span>Presione un elemento para ver detalles</span>
        <br><br>
        <details v-for="(item, index) in historialFacturasList" :key="index" :open="historialFacturasList[0].idfactura !== '-'">
            <summary style="display: flex; justify-content: space-between;" >
                <span>{{ item.fecha }} - Factura #{{ item.idFactura }} </span>
                <div id="icons">
            ...
            <div class="card-content">
                <span><b>Codigo cliente:</b> {{ item.cedula }}</span>
                <br>
...
export default {
    ...
    created() {
        fetch(`${BASE_URL}/facturas?codigoEstablecimiento=${this.establecimiento.codigoEstablecimiento}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                this.historialFacturasList = data;
            })
            .catch(error => {
